...
def update_dropdown_year(n_intervals):
    ...
    dropdown_options = [{'label': 'All', 'value': 'All'}] + [{'label': str(year), 'value': year} for year in updated_df['year'].unique()]
    return [dropdown_options]  # Đặt danh sách trong dấu ngoặc vuông để trả về một danh sách
...
@app.callback(
        Output('dashboarb-total', 'children'),
        [Input('interval-component', 'n_intervals')]
    )

def update_total_dashboarb(n_intervals, selected_business = 'all', selected_year = 'all', selected_category = 'all'):
    # Prevent initial callback trigger and subsequent ones if no changes in dropdowns
    df = update_dataframe()

    # filtered_df = df[(df['reviewerName'] == selected_business) & (df['reviewTime'].dt.year == selected_year) & (df['categories'] == selected_category)]

    html = create_dashboard_total_layout(df)
...
def update_chart_dashboarb(n_intervals):
    df = update_dataframe()
...
def update_chart_star_dashboarb(n_intervals):
    df = update_dataframe()
